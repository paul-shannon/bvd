...
Neither Javascript nor HTML, however, provide any native mechanism for class inheritance.  So in order to create
the web component of a new visualization application, there is unfortunately no alternative to copying the 
\Biocpkg{BrowserVizDemo} \emph{inst/scripts/demo.html} file into your new package's \emph{inst/scripts} 
directory, preserving everything that is of use, then adding your own HTML markup, CSS, Javascript
and probably jQuery, Boostrap and d3.  The copied Javascript code, like the corresponding code in R,
establishes the websocket connection and is easily configured to dispatch incoming messages to 
